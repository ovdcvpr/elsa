...
from pandas._typing import FilePath, WriteBuffer, StorageOptions
from typing import *
...
class Frame(
    ...
    def to_parquet(
        ...
        result = GeoDataFrame.to_parquet(
            self,
            path=path,
            *args,
            **kwargs,
        ...
        combined = {
            **old,
            b'magic': json.dumps(new).encode('utf-8')
        }
        pq.write_table(table.replace_schema_metadata(combined), path)
        return result

    def __getitem__(self, item) -> Self:
        # geopandas.GeoDataFrame.__getitem__ is hard-coded to cast the
        # result to a GeoDataFrame, DataFrame, or GeoSeries.
        # result = super().__getitem__(item)
        result = super(Frame, self).__getitem__(item)
        if issubclass(self.__class__, result.__class__):
    ...
    def __subset__(self, instance, value):
        ...
        if (
            ...
            if isinstance(value, GeoSeries):
                value = value.rename('geometry')
            else:
                value = GeoSeries(value, name='geometry', index=owner.index)
            # owner.set_geometry(value, inplace=True)
            owner['geometry'] = value
            try:
